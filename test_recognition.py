...
class TestRecognition(unittest.TestCase):
    ...
    def test_bing_french(self):
        ...
        r = sr.Recognizer()
        self.assertEqual(r.recognize_bing(audio, key=os.environ["BING_KEY"], language="fr-FR"), u"Essaye la dictÃ©e numÃ©ro un.")

    @unittest.skipUnless("BING_KEY" in os.environ, "requires Microsoft Bing Voice Recognition key to be specified in BING_KEY environment variable")
    def test_bing_chinese(self):
        audio = sr.AudioData.from_file(self.AUDIO_FILE_ZH)
        r = sr.Recognizer()
        self.assertEqual(r.recognize_bing(audio, key=os.environ["BING_KEY"], language="zh-CN"), u"ç ¸è‡ªå·±çš„è„šã€‚")
    ...
    def test_ibm_french(self):
        audio = sr.AudioData.from_file(self.AUDIO_FILE_FR)
        r = sr.Recognizer()
        self.assertEqual(r.recognize_ibm(audio, username=os.environ["IBM_USERNAME"], password=os.environ["IBM_PASSWORD"], language="fr-FR"), u"si la dictÃ©e numÃ©ro un ")

    @unittest.skipUnless("IBM_USERNAME" in os.environ and "IBM_PASSWORD" in os.environ, "requires IBM Speech to Text username and password to be specified in IBM_USERNAME and IBM_PASSWORD environment variables")
    def test_ibm_chinese(self):
        audio = sr.AudioData.from_file(self.AUDIO_FILE_ZH)
        r = sr.Recognizer()
        self.assertEqual(r.recognize_ibm(audio, username=os.environ["IBM_USERNAME"], password=os.environ["IBM_PASSWORD"], language="zh-CN"), u"ç ¸ è‡ªå·± çš„ è„š ")
